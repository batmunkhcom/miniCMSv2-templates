...
function getInsertPositionId($jeton_id) {
	global $DB,$DB2;
	...
	if ($DB->mbm_num_rows ( $r ) < 2) {
		...
		for($i = 0; $i < $DB->mbm_num_rows ( $r ); $i ++) {
			$q2 = "SELECT * FROM " . $DB->prefix . "jeton" . " WHERE jeton_id='" . $DB->mbm_result ( $r, $i, "id" ) . "'";
			$r2 = $DB->mbm_query ( $q2 );
			if ($DB->mbm_num_rows ( $r2 ) < 2) {
				return $DB->mbm_result ( $r, $i, "id" );
			} else {
				for($ii = 0; $ii < $DB->mbm_num_rows ( $r2 ); $ii ++) {
					$q3 = "SELECT * FROM " . $DB->prefix . "jeton" . " WHERE jeton_id='" . $DB->mbm_result ( $r2, $ii, "id" ) . "'";
					$r3 = $DB->mbm_query ( $q3 );
					if ($DB->mbm_num_rows ( $r3 ) < 2) {
						return $DB->mbm_result ( $r2, $ii, "id" );
					} else {
						/*for($iii = 0; $iii < $DB->mbm_num_rows ( $r3 ); $iii ++) {
							$q4 = "SELECT * FROM " . $DB->prefix . "jeton" . " WHERE jeton_id='" . $DB->mbm_result ( $r3, $iii, "id" ) . "'";
							$r4 = $DB->mbm_query ( $q4 );
...
function copyJeton($copy_username = '') {
	global $DB;
	$jeton_id = getJetonIdByUsername ( $copy_username );
	if ($jeton_id == 0) {
		return 0;
	}
	if (checkIsFull ( $jeton_id ) == 1) {
		
		$sponsor_jeton_id = $DB->mbm_get_field ( $jeton_id, 'id', 'jeton_id', 'jeton' ); //deed jeton id

		//deed jetonii hooson zaig oloh
		$insertId = getInsertPositionId ( $sponsor_jeton_id );
		$username = $DB->mbm_get_field ( $jeton_id, 'id', 'username', 'jeton' );
		...
		$data ['cnt'] = getMaxCount ( $username ) + 1;
...
function jetonBonus($jeton_id = 0) {
	global $DB;
	
	$sponsor1_jeton_id = $DB->mbm_get_field ( $jeton_id, "id", "jeton_id", "jeton" );
	$sponsor2_jeton_id = $DB->mbm_get_field ( $sponsor1_jeton_id, "id", "jeton_id", "jeton" );
	
	if (jetonTotalParentChilds ( $jeton_id ) == 2) {
		makeBonusPayment ( array (
								  'type' => 1, 
								  'amount' => BONUS_AMOUNT, 
								  'comment' => 'bonus', 
								  'user_id' => $DB->mbm_get_field ( $jeton_id, "id", "user_id", "jeton" ), 
								  'jeton_id' => $jeton_id, 'date_added' => mbmTime (), 
		...
		makeBonusPayment ( array (
								  ...
								  'amount' => BONUS_AMOUNT, 
								  'comment' => 'bonus', 
								  'user_id' => $DB->mbm_get_field ( $sponsor1_jeton_id, "id", "user_id", "jeton" ), 
								  'jeton_id' => $sponsor1_jeton_id, 'date_added' => mbmTime (), 
								  'ip' => getenv ( "REMOTE_ADDR" ), 
								  'browser' => mbmGetBrowser () 
								  ) 
						  );
